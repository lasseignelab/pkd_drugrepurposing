...
Save TSEA tables
```{r }
write.csv(tsea_GO_39@result, file = here("res", "sigsearch_outputs", "tsea_go_39.csv"))

write.csv(tsea_KEGG_39@result, file = here("res", "sigsearch_outputs", "tsea_kegg_39.csv"))
```
...
write.csv(gene_drug_targets, file = here("res", "sigsearch_outputs", "gene_drug_targets_39.csv"))
```
...
plot
```{r}
moa_alluvial <- ggplot(data = cystic_targets,
       aes(axis1 = pert, axis2 = t_gn_sym.x)) +
  geom_alluvium(color = "black", aes(fill = WTCS.x)) + # , colour = trend)) + 
  geom_stratum(width = 3/12, color = "black") + #the border of the stratum
  scale_x_discrete(expand = c(.07, .07)) + 
  scale_y_discrete(expand = c(.007, .007)) + 
  scale_fill_viridis_c(aesthetics = c("colour", "fill")) +
...
fda_approved_39_moa$MOA <- stringr::str_wrap(fda_approved_39_moa$MOA, width = 40)

#rhub_launched_39_top_moasep$Disease.Area <- stringr::str_wrap(rhub_launched_39_top_moasep$Disease.Area, width = 30)
fda_approved_39_moa_top <- dplyr::slice_min(fda_approved_39_moa, order_by = WTCS, n = 38)
```
...
```{r }
ggsave(here("res", "sigsearch_outputs", "moa_fda_alluvial_220824.pdf"), moa_alluvial, width = 17, height = 10)
...
#visualize
moa_frq <- ggplot(moa_freq, aes(x = Var1, y = Freq, fill = Freq)) +  # Plot with values on top
  geom_bar(stat = "identity") + 
  scale_fill_viridis_c(aesthetics = c("colour", "fill")) +
  #geom_text(aes(label = Var1), vjust = 0) + 
  theme(axis.text.x = element_text(angle = 50, vjust = 0.90, hjust=0.75)) + ggtitle(label = "Most Frequent MOA's")

moa_frq
```

```{r}
ggsave(filename = here("res/sigsearch_outputs", "p70_fdaapproved_moa_freq.pdf"), moa_frq)
...
#visualize
indication_plot <- ggplot(indication_freq, aes(x = Var1, y = Freq, fill = Freq)) +  # Plot with values on top
  geom_bar(stat = "identity") + 
  scale_fill_viridis_c(aesthetics = c("colour", "fill")) +
  #geom_text(aes(label = Var1), vjust = 0) + 
  theme(axis.text.x = element_text(angle = 50, vjust = 0.90, hjust=0.75)) + ggtitle(label = "Most Frequent Disease Area Indications")
...
plot
```{r}
target_alluvial <- ggplot(data = target_tbl,
       aes(axis1 = pert, axis2 = Upregulated_Target)) +
  geom_alluvium(color = "black", aes(fill = Upregulated_Target), show.legend=FALSE) + # , colour = trend)) + 
  geom_stratum(width = 3/12, color = "black") + #the border of the stratum
  scale_x_discrete(expand = c(.07, .07)) + 
  scale_y_discrete(expand = c(.007, .007)) + 
...
```{r }
#target-separated, rhub-annotated drug results for 39
write.csv(rhub_launched_39_septarget, file = here("res", "sigsearch_outputs", "gene_drug_targets_39.csv"))
```

Drug data for drugs whose targets are upregulated in both cystic data sets
```{r}
cystic_overlap_drugs <- dplyr::filter(fda_approved_39, pert %in% unique(cystic_targets$pert))
...
```{r}
drug_sigs <- getDEGSig(cmp = "fluoxetine", cell = "HA1E", Nup = 50, Ndown = 50,  padj = 0.05, refdb = lincs)
...
```{r}
function (table_list, rank_stat = "pvalue", Nshow = 20, Nchar = 50, 
    scien = FALSE, ...) 
{
    ...
    newtb <- function(i) {
        ...
        tb2 <- data.frame(tb_order[, c("Description", rank_stat)], 
            Method = names(table_list)[i], rank = seq_len(nrow(tb_order)))
        return(tb2)
    }
    ...
    df2$Description <- factor(df2$Description, levels = rev(cat_top), 
        ordered = TRUE)
    p <- ggplot(df2, aes_(x = ~Method, y = ~Description)) + geom_point(size = 4, 
        ...) + aes_string(color = rank_stat) + scale_colour_gradient(low = "red", 
        high = "blue") + theme_dose(font.size = 12) + ggplot2::theme(axis.text.x = element_text(angle = 45, 
        hjust = 1))
    return(p)
}
...
```{r}
drugsig <- function(cmp) {
  res <- getDEGSig(cmp = cmp, cell = "HA1E", Nup = 50, Ndown = 50,  padj = 0.05, refdb = lincs)
  return(res)
}
t <- apply(cystic_overlap_drugs, "pert", drugsig)
```
...
```{r}
onsides_res <- dplyr::left_join(cystic_overlap_drugs, labels, by = c("pert" = "ingredients"))
...
prioritized_drugs <- dplyr::setdiff(cystic_overlap_drugs$pert, renal_liver_ade$pert)
prioritized_drugs <- dplyr::filter(cystic_overlap_drugs, pert %in% prioritized_drugs)
```
...
write.csv(simple_tbl, file = here("res/sigsearch_outputs", "drugtarget_cysticoverlap_simpletbl.csv"))
```
