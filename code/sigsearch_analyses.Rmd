...
```{r}
pravastatin_sig <- getDEGSig(cmp = "pravastatin", cell = "HA1E", Nup = 100, Ndown = 100,  padj = 0.05, refdb = lincs)
...
```{r}
bubbleplot(pravastatin_fea$topcompared)
...
```{r}
dsea_prioritzeddrugs@result$Description <- stringr::str_wrap(dsea_prioritzeddrugs@result$Description, width = 75)

dseaplot <- ggplot(dplyr::slice_min(dsea_prioritzeddrugs@result, order_by = p.adjust, n = 20), aes(x=Count, y=Description, size = Count, fill = p.adjust)) +
...
```
```{r}
ggsave(filepath, dseaplot, width = 10, height = 7)
```

drug-target networks for the terms with most drugs
```{r}
dtnetplot(drugs = drugs(dsea_prioritzeddrugs), set = "GO:0016712", ont = "MF", 
          desc="oxidoreductase activity, acting on paired donors, with incorporation or reduction of molecular oxygen...")

```


```{r}
dtnetplot(drugs = drugs(dsea_prioritzeddrugs), set = "GO:1901338", ont = "MF", 
