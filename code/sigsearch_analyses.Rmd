...
write.csv(gess_kidneycells_39, file = here("res", "sigsearch_outputs", "gess_kidneycells_39_220421.csv"))
...
fda_unique <- distinct(fda_data, DrugName, .keep_all = TRUE)
fda_approved <- filter(fda_unique, MarketingStatusDescription != "Discontinued")
```


```{r}
#test <- inner_join(rhub_launched_39, fda_unique, by = c("pert", "ActiveIngredient"), suffix = c("rhub", "fda"))

#test <- merge(rhub_launched_39, fda_unique, by.x = "pert", by.y = tolower("ActiveIngredient"))
```
...
```{r eval=FALSE, include=FALSE}
ggsave(here("res", "sigsearch_outputs", "moa_alluvial_wtcs_2.pdf"), moa_alluvial, width = 10, height = 15)
...
```{r}
fda_approved_rhub39 <- c("loperamide",
"etoposide",
"irinotecan",
"methylene-blue",
"ixazomib",
