...
Alluvial plot of drugs-targets with just launched drugs
```{r}
rhub_launched_39_top30 <- dplyr::slice_min(fda_approved_39, order_by = Tau, n = 30)
...
rm later -- old exercise mimetic results
```{r}
ex_ss <- read.csv("/Users/eramsey/Desktop/R21_210302/PreliminaryR21/signatureSearch_outputs/210526_lincs_human_res.csv", row.names = 1)
ex_mimetics <- dplyr::filter(ex_ss, trend == "up")
#number of unique compounds
length(unique(ex_mimetics$pert))
```

filter for ones available in repurp hub
```{r}
ex_mimetics_annot <- dplyr::inner_join(ex_mimetics, repurpHub, by = c("pert" = "Name"))
length(unique(ex_mimetics_annot$pert))
```

look for anti-depressants
```{r}
depr_terms <- agrep( "depression", ex_mimetics_annot$Indication, ignore.case = TRUE, value = TRUE)
ex_antidep <- dplyr::filter(ex_mimetics_annot, Indication %in% depr_terms)
```

filter for launched
```{r}
ex_mimetics_launched <- dplyr::filter(ex_mimetics_annot, Phase == "Launched")
length(unique(ex_mimetics_launched$pert))
```


MOA's separated, string wrap labels
```{r}
ex_mimetics_launched <- tidyr::separate_rows(ex_mimetics_launched, MOA, sep = ",")
ex_mimetics_launched <- dplyr::filter(ex_mimetics_launched, MOA != "")
ex_mimetics_launched$MOA <- stringr::str_wrap(ex_mimetics_launched$MOA, width = 35)
...
plot
```{r}
moa_alluvial <- ggplot(data = dplyr::slice_max(ex_mimetics_launched, order_by = NCS, n = 35),
       aes(axis1 = pert, axis2 = MOA)) +
  geom_alluvium(color = "black", aes(fill = NCS)) + # , colour = trend)) + 
  geom_stratum(width = 1/3, color = "black") + #the border of the stratum
  scale_x_discrete(expand = c(.07, .07)) + 
  scale_y_discrete(expand = c(.007, .007)) + 
  scale_fill_viridis_c(aesthetics = c("colour", "fill")) +
  geom_text(stat = "stratum", size = 3.5, fontface = "bold", aes(label = after_stat(stratum))) +
...
```{r}
gess_res_vis(ex_ss, drugs = unique(gess_kidneycells_39$pert)[1:20], col = "NCS")
```

save results
```{r}
write.csv(ex_mimetics_annot, "/Users/eramsey/Desktop/R21_210302/PreliminaryR21/signatureSearch_outputs/human_exercise_sigsearch_mimetics.csv")

write.csv(ex_antidep, "/Users/eramsey/Desktop/R21_210302/PreliminaryR21/signatureSearch_outputs/human_exercise_sigsearch_mimetics_antidepressants.csv")


ggsave("/Users/eramsey/Desktop/R21_210302/PreliminaryR21/signatureSearch_outputs/human_exercise_sigsearch_mimetics_top30launched_alluvial.pdf",moa_alluvial, width = 12, height = 12)
```
