...
```{r}
ggsave(filename =
         here("res/sigsearch_outputs", "p70_fdaapproved_origindication_freq.pdf"), indication_plot, height = 5.5, width = 8)
...
dsea_plot <- dsea_prioritzeddrugs %>% dplyr::slice_min(order_by = p.adjust, n = desc(20)) %>% dplyr::mutate(Description = forcats::fct_reorder(Description, Count)) %>%  ggplot( aes(x=Count, y=Description, fill = p.adjust)) +
  geom_bar( stat = "identity") + 
  scale_fill_viridis_c(begin = 0.35, end = 0.95) + 
