...
treemapPlot2 <- function (reducedTerms, size = "score", title = "", ...) 
{
    ...
    treemap::treemap(reducedTerms, index = c("parentTerm", "term"), 
        vSize = size, type = "index", title = title, palette = gg_color_hue(length(unique(reducedTerms$parent))), 
...
simMatrix_56 <- calculateSimMatrix(go_56$term_id, orgdb = "org.Hs.eg.db", ont = "BP", method = "Wang")
...
simMatrix_19 <- calculateSimMatrix(go_19$term_id, orgdb = "org.Hs.eg.db", ont = "BP", method = "Wang")

scores_19 <- setNames(-log10(go_19$p_value), go_19$term_id)
reducedTerms_19 <- reduceSimMatrix(simMatrix_19, scores_19, threshold=0.8, orgdb="org.Hs.eg.db")
...
```{r}
fea_mouse_39 <- mouseenrich(musdegs_39_UP$ensembl_gene_id, musdegs_39_DOWN$ensembl_gene_id, 30)
