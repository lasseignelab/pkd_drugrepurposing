...
```{r}
combined_rlog <- merge(all_56, all_19, by = "X")
combined_rlog <- merge(all_39, combined_rlog, by = "X")

rlog_aki_up <- combined_rlog %>% dplyr::filter( X %in% aki_upgenes) %>% dplyr::mutate(direction = "aki_up") %>% dplyr::select(Genes = X, PrecysticP70 = log2FoldChange, CysticP21 = log2FoldChange.x, CysticP28 = log2FoldChange.y, Direction = direction)#152
rlog_aki_down <- combined_rlog %>% dplyr::filter( X %in% aki_downgenes) %>% dplyr::mutate(direction = "aki_down") %>% dplyr::select(Genes = X, PrecysticP70 = log2FoldChange, CysticP21 = log2FoldChange.x, CysticP28 = log2FoldChange.y, Direction = direction)
...
#show_col(viridis(4, option = "E")) #selecting colors
...
merge for overlap
```{r}
fea_cystic_overlap_up <- merge(fea_mouse_56_upterms, fea_mouse_19_upterms, by = "term_name")

fea_cystic_overlap_down <- merge(fea_mouse_56_downterms, fea_mouse_19_downterms, by = "term_name")
```
