...
mouseenrich <- function(upgenes, downgenes, topreturned, sources = c("GO:BP", "GO:MF", "GO:CC", "REAC", "WP")) {
  ...
  up_fea <- fea(upgenes$Mouse.gene.stable.ID, sources = sources)
  ...
  down_fea <- fea(downgenes$Mouse.gene.stable.ID, sources = sources)
...
```{r}
p70_uniquegene_enrich <- enrich(p70_unique_up, p70_unique_down, 30)
...
bp_39 <- bubbleplot(fea_39$topcompared) + labs(title = "Top Enriched Terms for Precystic P70 Signature") + theme(text = element_text(size = 12), plot.title = element_text(size = 18))

bp_39
```

```{r}
ggsave(bp_39, filename = here("res", "fea", "bubbleplot_signature_p70.pdf"), height = 7, width = 12)
...
bp_56 <- bubbleplot(fea_56$topcompared) + labs(title = "Top Enriched Terms for Cystic P21 Signature") + theme(text = element_text(size = 12), plot.title = element_text(size = 18))

bp_56
```

```{r}
ggsave(bp_56, filename = here("res", "fea", "bubbleplot_signature_p21.pdf"), height = 7, width = 12)
...
```{r}
fea_19$topcompared$term_name <- stringr::str_wrap(fea_19$topcompared$term_name, width = 80)

bp_19 <- bubbleplot(fea_19$topcompared) + labs(title = "Top Enriched Terms for Cystic P28 Signature") + theme(text = element_text(size = 12), plot.title = element_text(size = 18))
...
```{r}
ggsave(bp_19, filename = here("res", "fea", "bubbleplot_signature_p28.pdf"), height = 7, width = 12)
...
##### rrvgo
alter their scatterplot function
```{r}
...
```{r}
degs_19_56_overlap_up <- merge(musdegs_19_UP, musdegs_56_UP, by = "ensembl_gene_id")

degs_19_56_overlap_down <- merge(musdegs_19_DOWN, musdegs_56_DOWN, by = "ensembl_gene_id")

degs_39_56_overlap_up <- merge(musdegs_39_UP, musdegs_56_UP, by = "ensembl_gene_id")

degs_39_56_overlap_down <- merge(musdegs_39_DOWN, musdegs_56_DOWN, by = "ensembl_gene_id")


degs_39_19_overlap_up <- merge(musdegs_39_UP, musdegs_19_UP, by = "ensembl_gene_id")

degs_39_19_overlap_down <- merge(musdegs_39_DOWN, musdegs_19_DOWN, by = "ensembl_gene_id")


degs_all_overlap_up <- merge(degs_19_56_overlap_up, musdegs_39_UP, by = "ensembl_gene_id")

degs_all_overlap_down <- merge(degs_19_56_overlap_down, musdegs_39_DOWN, by = "ensembl_gene_id")
...
```{r}
#at least half a lfc and padj > 0.1 
#all_39 <- dplyr::filter(all_39, log2FoldChange > 0.5 | log2FoldChange < -0.5 & padj < 0.1)
#all_56 <- dplyr::filter(all_56, log2FoldChange > 0.5 | log2FoldChange < -0.5 & padj < 0.1)
#all_19 <- dplyr::filter(all_19, log2FoldChange > 0.5 | log2FoldChange < -0.5 & padj < 0.1)
...
rlog_aki_up <- combined_fc %>% dplyr::filter( X %in% aki_upgenes) %>% dplyr::mutate(direction = "aki_up") %>% dplyr::select(Genes = X, PrecysticP70 = log2FoldChange, CysticP21 = log2FoldChange.x, CysticP28 = log2FoldChange.y, Direction = direction)#152

rlog_aki_down <- combined_fc %>% dplyr::filter( X %in% aki_downgenes) %>% dplyr::mutate(direction = "aki_down") %>% dplyr::select(Genes = X, PrecysticP70 = log2FoldChange, CysticP21 = log2FoldChange.x, CysticP28 = log2FoldChange.y, Direction = direction)

aki_res <- rbind(rlog_aki_up, rlog_aki_down)
```
...
#my_color = list( aki_direction = c(aki_down = "#575C6DFF", aki_up = "#FFEA46FF"), samples = c(Precystic = "#00204DFF", Cystic = "#A69D75FF"))
#my_color = list(Direction = c(aki_down = "#575C6DFF", aki_up = "#FFEA46FF"), Cystic_Status = c(Precystic = "#00204DFF", Cystic = "#A69D75FF"))


pdf(here("res/deseq2_outputs", "heatmap_aki_genes.pdf"), width = 9, height = 11)
#aki_heatmap <- 
  pheatmap(aki_res2, annotation_colors = my_color, annotation_row = aki_direction, annotation_col = samples, cluster_rows = TRUE, cluster_cols = TRUE, show_rownames = FALSE, color = viridis(n= 10000, alpha = 1, begin = 0, end = 1, option = "viridis"), border_color = "NA", main = "Expression of Acute Kidney Injury-Associated Genes Across Precystic/Cystic Data ", angle_col = "0")
dev.off()
```
...
ggsave(aki_heatmap, filename = here("res/deseq2_outputs", "heatmap_aki_genes.pdf"), height = 7, width = 14)
```
...
venn.diagram(x = list(mus_set39$ensembl_gene_id, mus_set56$ensembl_gene_id, mus_set19$ensembl_gene_id),
        category.names = c("Precystic P70" , "Cystic P21" , "Cystic P28"),
...
cystic up overlap
```{r}
ggplot(slice_max(fea_cystic_overlap_up, order_by = recall.y, n = 20), aes(x=intersection_size.y, y=term_name,  fill = p_value.y)) + 
    geom_bar( stat = "identity") + 
    scale_fill_continuous(type = "viridis") + 
...
cystic down overlap
```{r}
ggplot(slice_max(fea_cystic_overlap_down, order_by = recall.y, n = 20), aes(x=intersection_size.y, y=term_name,  fill = p_value.y)) + 
    geom_bar( stat = "identity") + 
    scale_fill_continuous(type = "viridis") + 
    labs(x = "Number of Genes Matched to Term", y = "Functional Terms", title = "Top Enriched Terms") + 
    theme_minimal(base_size = 8)
...
#test <- fea_cystic_overlap_combined <- fea_cystic_overlap_combined %>% rowwise() %>% mutate(total_genes = sum(intersection_size.x, intersection_size.y), avg_recall = mean(recall.x, recall.y), avg_pvalue = mean(p_value.x, p_value.y))

#average of recall
#fea_cystic_overlap_combined <- fea_cystic_overlap_combined %>% rowwise() %>% mutate(avg_recall = mean(recall.x, recall.y))

#average of p-value
#fea_cystic_overlap_combined <- fea_cystic_overlap_combined %>% rowwise() %>% mutate(avg_pvalue = mean(p_value.x, p_value.y))

test <- dplyr::slice_min(fea_cystic_overlap_combined, order_by = "p_value.x", n = 10)
#test <- dplyr::slice_tail(fea_cystic_overlap_combined, order_by = p_value.x, n = 10)
test <- arrange(fea_cystic_overlap_combined, desc("total_genes"))#[1:10,]
```

```{r}
ggplot(slice_max(fea_cystic_overlap_combined, order_by = total_genes, n = 10), aes(x=direction.x, y=term_name, size = total_genes, fill = p_value.y)) +
  geom_point(alpha=0.7, shape = 21) +
  scale_size(range = c(2, 10), name = "# Genes Matched to Term") + 
...
sp_56all
tm_56all <- treemapPlot(reducedTerms_56all, title = "Enrichment for Cystic P21 Signature")
...
sp_19all
tm_19all <- treemapPlot(reducedTerms_19all, title = "Enrichment for Cystic P21 Signature")
...
```{r}
go_term_heatmap<- function(limma_pathways, deseq2_pathways, TFL_pathways, direction, threshold = 0.95){
  #input
  ...
  #get terms
  limma_bp<- limma_pathways$term_id[ limma_pathways$source == "GO:BP" & limma_pathways$direction == direction]
  deseq2_bp <- deseq2_pathways$term_id[ deseq2_pathways$source == "GO:BP" & deseq2_pathways$direction == direction]
  TFL_bp <- TFL_pathways$term_id[ TFL_pathways$source == "GO:BP" & TFL_pathways$direction == direction]
  ...
  #create heatmap
  row_ha = HeatmapAnnotation(CysticP28=tfl_drugs,CysticP21= deseq2_drugs, PrecysticP70= limma_drugs,GO_BP_Group= res_v2$parentTerm , col = list(CysticP28 = c("TRUE" = "#fde725", "FALSE" = "#440154"),CysticP21 = c("TRUE" = "#fde725", "FALSE" = "#440154"),PrecysticP70 = c("TRUE" = "#fde725", "FALSE" = "#440154"), GO_BP_Group= bp_color ))
...
#limma_pathways, deseq2_pathways, TFL_pathways
```


```{r}
#go_term_heatmap(limma_pathways, deseq2_pathways, TFL_pathways, "Up")
comb_39 <- mouseenrich(musdegs_39_UP, musdegs_39_DOWN, 100, sources = "GO:BP")

comb_56 <- mouseenrich(musdegs_56_UP, musdegs_56_DOWN, 100,  sources = "GO:BP")

comb_19 <- mouseenrich(musdegs_19_UP, musdegs_19_DOWN, 100,  sources = "GO:BP")
```


```{r}
pdf(here("res/fea", "alldata_musgenes_gosim_heatmap.pdf"), width = 12, height = 8)
