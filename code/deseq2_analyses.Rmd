...
```{r}
musdegs_39_UP <- uplfc(dsq2_39, lfcutoff = 1.3)
musdegs_39_DOWN <- downlfc(dsq2_39, lfcutoff = -1.3)

```

Total genes above threshold before mapping from mouse ensembl to human symbol/entrez
```{r}
dim(musdegs_39_UP) #115 at lfc > 2 #172 at 1.5 #206 at 1.3
dim(musdegs_39_DOWN) #44 at lfc < -2 #71 at -1.5 #89 at 1.3
```
...
dim(hom_degs_UP) #55 at lfc >2 #89 at lfc > 1.5 #100 at 1.3
dim(hom_degs_DOWN) #15 at lfc <-2 #27 at lfc < -1.5 #37 at 1.3
```
...
heatmap_p70 <- pheatmap(sig_39_cts[row.names(sig_39_cts), ], cluster_rows = TRUE, cluster_cols = TRUE, show_rownames = FALSE, color = viridis(n= 10000, alpha = 1, begin = 0, end = 1, option = "viridis"), border_color = "NA", main = "Gene Signature for Pre-Cyst P70")
...
targets in > 2LFC
```{r}
#targets_in39 <- merge(targets_39, sig_39, by.x = "t_gn_sym", by.y = "hgnc_symbol")
#volc_39 <- merge(conv_cts_all, )

#volc_39 <- tibble:::column_to_rownames(conv_cts_all, "hgnc_symbol")
#targets_in39 <- dplyr::filter(targets_in39, log2FoldChange >  2.0)# | log2FoldChange < -2.0)
#test <- cbind(targets_in39$t_gn_sym, targets_in39$LFC)
```
...
heatmap_p21 <- pheatmap(sig_56_cts[row.names(sig_56_cts), ], cluster_rows = TRUE, cluster_cols = TRUE, show_rownames = FALSE, color = viridis(n= 10000, alpha = 1, begin = 0, end = 1, option = "viridis"), border_color = "NA", main = "Gene Signature for Cystic P21", angle_col = 45)
...
heatmap_p28 <- pheatmap(sig_19_cts[row.names(sig_19_cts), ], cluster_rows = TRUE, cluster_cols = TRUE, show_rownames = FALSE, color = viridis(n= 10000, alpha = 1, begin = 0, end = 1, option = "viridis"), border_color = "NA", main = "Gene Signature for Cystic P28", angle_col = 45)
