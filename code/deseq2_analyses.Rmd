...
gene_desc <- getBM(filters = "ensembl_gene_id", attributes = c("ensembl_gene_id","entrezgene_id", "mgi_symbol", "description"), values = cts_df$gene_id, mart = mouse)
...
conv_cts <- merge(cts_df, coding_genes, by = "ensembl_gene_id")

#Filter out repeating data
conv_cts <- dplyr::distinct(conv_cts, mgi_symbol, .keep_all = TRUE)
#Convert back to rownames but with gene symbols
conv_cts <- tibble:::column_to_rownames(conv_cts, "mgi_symbol")
...
```{r}
ggsave(heatmap_p70, filename = here("res", "fea", "heatmap_p70_ribopseudoremoved.pdf"), height = 7, width = 14)
```
...
targets in > 2LFC
```{r}
targets_in39 <- merge(targets_39, conv_tbl, by.x = "t_gn_sym", by.y = "hgnc_symbol")
targets_in39 <- dplyr::filter(targets_in39, log2FoldChange >  2.0)# | log2FoldChange < -2.0)
```

```{r}
volcano_39 <- EnhancedVolcano(conv_tbl, lab = conv_tbl$hgnc_symbol, x = 'log2FoldChange', y = 'padj', selectLab = unique(targets_in39$t_gn_sym), pCutoff = 0.05, FCcutoff = 0.5, pointSize = 3.0, labSize = 4.0, col = c("#440154", "#21918c", "#5ec962", "#fde725"), title = "Drug Targets Found in P70 Pre-Cystic DEGs", labFace = 'bold', boxedLabels = TRUE,drawConnectors = TRUE,widthConnectors = 0.5,colConnectors = 'black')


EnhancedVolcano(conv_tbl, lab = conv_tbl$hgnc_symbol, x = 'log2FoldChange', y = 'padj', pCutoff = 0.05, FCcutoff = 2, pointSize = 3.0, labSize = 4.0, col = c("#440154", "#21918c", "#5ec962", "#fde725"), title = "Drug Targets Found in P70 Pre-Cystic DEGs", labFace = 'bold', drawConnectors = TRUE,widthConnectors = 0.5,colConnectors = 'black')
volcano_39
```

```{r}
ggsave(volcano_39, filename =  here("res", "deseq2_outputs", "volcano_drugtargets_39degs.pdf"))
```
...
#Merge gene symbols into counts and gene ID
conv_cts_56 <- merge(cts_df_56, coding_genes, by = "ensembl_gene_id")
#Filter out repeating data
conv_cts_56 <- dplyr::distinct(conv_cts_56, mgi_symbol, .keep_all = TRUE)
#Convert back to rownames but with gene symbols
conv_cts_56 <- tibble:::column_to_rownames(conv_cts_56, "mgi_symbol")
...
```{r eval=FALSE, include=FALSE}
ggsave(heatmap_21, filename = here("res", "fea", "heatmap_p21_ribopseudoremoved.pdf"), height = 7, width = 14)
...
targets in > 2LFC
```{r}
targets_in56 <- merge(targets_39, conv_tbl_56, by.x = "t_gn_sym", by.y = "hgnc_symbol")
targets_in56 <- dplyr::filter(targets_in56, log2FoldChange >  2.0 )#| log2FoldChange < -2.0)
```


```{r}
volcano_56 <- EnhancedVolcano(conv_tbl_56, lab = conv_tbl_56$hgnc_symbol, x = 'log2FoldChange', y = 'padj', selectLab = unique(targets_in56$t_gn_sym), pCutoff = 0.05, FCcutoff = 0.5, pointSize = 3.0, labSize = 4.0, col = c("#440154", "#21918c", "#5ec962", "#fde725"), title = "P70 Pre-Cystic Drug Targets Found in P21 \nCystic DEGs", labFace = 'bold', boxedLabels = TRUE,drawConnectors = TRUE,widthConnectors = 0.5,colConnectors = 'black', max.overlaps = 50)

volcano_56
```

```{r}
ggsave(volcano_56, filename =  here("res", "deseq2_outputs", "volcano_drugtargets_56degs.pdf"))
```
...
#Merge gene symbols into counts and gene ID
conv_cts_19 <- merge(cts_df_19, coding_genes, by = "ensembl_gene_id")
#Filter out repeating data
conv_cts_19 <- dplyr::distinct(conv_cts_19, mgi_symbol, .keep_all = TRUE)
#Convert back to rownames but with gene symbols
conv_cts_19 <- tibble:::column_to_rownames(conv_cts_19, "mgi_symbol")
#Remove unnecessary data
...
```{r eval=FALSE, include=FALSE}
ggsave(heatmap_p28, filename = here("res", "fea", "heatmap_p28_ribopseudoremoved.pdf"), height = 7, width = 14)
...
targets in > 2LFC
```{r}
targets_in19 <- merge(targets_39, conv_tbl_19, by.x = "t_gn_sym", by.y = "hgnc_symbol")
targets_in19 <- dplyr::filter(targets_in19, log2FoldChange >  2.0)# | log2FoldChange < -2.0)
```


```{r}
volcano_19 <- EnhancedVolcano(conv_tbl_19, lab = conv_tbl_19$hgnc_symbol, x = 'log2FoldChange', y = 'padj', selectLab = unique(targets_in19$t_gn_sym), pCutoff = 0.05, FCcutoff = 0.5, pointSize = 3.0, labSize = 4.0, col = c("#440154", "#21918c", "#5ec962", "#fde725"), title = "P70 Pre-Cystic Drug Targets Found in P28 \nCystic DEGs", labFace = 'bold', boxedLabels = TRUE,drawConnectors = TRUE,widthConnectors = 0.5,colConnectors = 'black', max.overlaps = 25) 
...
```{r}
ggsave(volcano_19, filename =  here("res", "deseq2_outputs", "volcano_drugtargets_19degs.pdf"))
```
