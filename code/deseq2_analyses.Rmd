...
test run
```{r}
test <- convertMouseGeneList_2(row.names(dsq2_39))
...
targets_in_degs <- function(conv_tbl, degs, targets){
  ...
  #Convert gene ID's of rlog normalized count data to human symbols
  #newcts <- merge(cts, conv_tbl, by.x = "ensembl_gene_id", by.y = "Mouse.gene.stable.ID")

  #targets expressed above cutoff
  #targets_in_degs <- merge(targets, conv_tbl, by.x = "t_gn_sym", by.y = "hgnc_symbol")
  #targets_in_degs <- dplyr::filter(targets_in_degs, log2FoldChange > cutoff)
...
target_deg_plot <- function(targetdegs, cutoff){
  ...
  volcanoplot <- EnhancedVolcano(targetdegs, lab = targetdegs$t_gn_sym, x = 'log2FoldChange', y = 'padj', selectLab = unique(target_labels$t_gn_sym), pCutoff = 0.05, FCcutoff = cutoff, pointSize = 3.0, labSize = 4.0, col = c("#440154", "#21918c", "#fde725", "#5ec962"), title = "P70 Pre-Cystic Drug Targets Found in P70 \nPre-Cystic DEGs", labFace = 'bold', boxedLabels = TRUE,drawConnectors = TRUE,widthConnectors = 0.5,colConnectors = 'black', max.overlaps = 25)
return(volcanoplot)
}
...
testing
```{r}
test2 <- targets_in_degs(test, dsq2_39, fda_approved_39_targets)

test3 <- test2 %>% dplyr::group_by(t_gn_sym) %>% tidyr::nest()

vp <- target_deg_plot(test2, 1.3)
vp
```
...
```{r}
volc_test <- EnhancedVolcano(test2, lab = test2$t_gn_sym, x = 'log2FoldChange', y = 'padj', selectLab = unique(test2$t_gn_sym), pCutoff = 0.05, FCcutoff = 1.3, pointSize = 3.0, labSize = 4.0, col = c("#440154", "#21918c", "#fde725", "#5ec962"), title = "P70 Pre-Cystic Drug Targets Found in P70 \nPre-Cystic DEGs", labFace = 'bold', boxedLabels = TRUE,drawConnectors = TRUE,widthConnectors = 0.5,colConnectors = 'black', max.overlaps = 25)
#"#440154", "#21918c", "#5ec962", "#fde725"
#"#fde725", "#5ec962","#21918c", "#440154"
volc_test
```
